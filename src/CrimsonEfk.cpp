...
EffekseerRefHandle EffekseerLoadEffect(const wchar_t* path, float scale)
{
...
EffekseerRefHandle EffekseerLoadEffectFromMemory(const void* data, int size, float scale)
{
...
EffekseerHandle EffekseerPlayEffect(EffekseerRefHandle hEffect, float x, float y, float z, void* player) {
...
void EffekseerRender(ID3D11DeviceContext* pContext, float deltaTime)
{
...
__int64 __fastcall D3D11_Draw_sub_140043F90(Devil3::sRender* render, unsigned int a2, unsigned int a3, unsigned __int64 a4, __int64 a5) {
    ...
    return original(render, a2, a3, a4, a5);
}

...
bool IsArchive(byte8* addr) {
    ...
    if (SignatureMatch(addr, signature1) || SignatureMatch(addr, signature2)) {
        return true;
    }
...
bool ExtractFile(const char* name) {
    ...
    snprintf(path, sizeof(path), "data\\dmc3\\dmc3-0.nbz");

    archive = zip_open(path, 0, &result.zip_err);
    if (!archive) {
    ...
    snprintf(path, sizeof(path), "GData.afs/%s", name);
    ...
    snprintf(path, sizeof(path), "data\\dmc3\\GData.afs\\%s", name);

    if (!SaveFile(path, addr, stats.size)) {
        Log("SaveFile failed.");
...
byte8* FileManager::Metadata::operator[](uint32 fileIndex) {
    ...
    if (!addr || (size == 0) || (alignedSize == 0) || !IsArchive(addr)) {
        if constexpr (debug) {
            Log("IsArchive failed.");
...
byte8* FileManager::Push(const char* name, bool skipArchive) {
    if constexpr (debug) {
        LogFunction();
    ...
    if (!dataAddr || (dataSize == 0) || !metadataAddr || (metadataSize == 0) || !name) {
        return 0;
    }
    ...
    file = OpenFile(path, FileFlags_Read);
...
byte8* FileManager::Push(byte8* addr, uint64 size) {
    if constexpr (debug) {
        LogFunction();
    ...
    if (!dataAddr || (dataSize == 0) || !metadataAddr || (metadataSize == 0) || !addr || (size == 0)) {
        return 0;
    }
...
FileManager File_staticFiles;
FileManager File_dynamicFiles;
...
void File_UpdateFileData(uint16 fileDataIndex, uint16 cacheFileIndex) {
    ...
    */
    ...
    File_UpdateFileData(fileData, cacheFileIndex);
}

FileData enemyFileData[ENEMY_FILE_DATA_COUNT]        = {};
FileDataMetadata enemyFileDataMetadata[ENEMY::COUNT] = {};

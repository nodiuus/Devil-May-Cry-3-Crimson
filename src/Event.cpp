...
void TriggerCustomizeMenu() {
    if (!activeConfig.Actor.enable || (g_shopTimer > 0)) {
        return;
    }
...
void CreateMainActor(byte8* actorBaseAddr) {
    ...
    SetMemory(g_eventRun, 0, sizeof(g_eventRun));
...
void UpdateEnemyCount() {
    auto pool_203 = *reinterpret_cast<byte8***>(appBaseAddr + 0xC90E28);
    if (!pool_203 || !pool_203[8]) {
        return;
    }
    auto& enemyVectorData = *reinterpret_cast<EnemyVectorData*>(pool_203[8]);
...
void EventHandler(EventData& eventData) {
    ...
    if ((event == EVENT::TELEPORT) && g_noTeleport) {
...
void IncStyleExpPoints(byte8* actorBaseAddr) {
    ...
    ExpConfig::Exp::IncStyleExpPoints(actorBaseAddr);
}
    ...
    // IncStyleExpPoints
    {
        ...
        */
        ...
        if (!run) {
            ...
            func = CreateFunction(IncStyleExpPoints, jumpAddr,
                (FunctionFlags_SaveRegisters | FunctionFlags_NoResult | FunctionFlags_SaveXMMRegisters | FunctionFlags_NoXMMResult), size);
    ...
    // MissionStartTriggerCustomizeMenu
    {
        ...
        */
        ...
        if (!run) {
            backupHelper.Save(addr, size);
            func = CreateFunction(
                MissionStartTriggerCustomizeMenu, jumpAddr, FunctionFlags_SaveRegisters, 0, sizeof(sect1), (sizeof(sect2) + size));
            CopyMemory(func.sect1, sect1, sizeof(sect1));
            CopyMemory(func.sect2, sect2, sizeof(sect2));
            CopyMemory((func.sect2 + sizeof(sect2)), addr, size, MemoryFlags_VirtualProtectSource);
